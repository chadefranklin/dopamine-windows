                    ...
                    Application.Current.Dispatcher.Invoke(() =>
                    {
                        ...
                        switch (this.AlbumOrder)
                        {
                            case AlbumOrder.Alphabetical:
                                for (insertIndex = 0; insertIndex < count && FormatUtils.GetSortableString(this.Albums[insertIndex].AlbumTitle).CompareTo(FormatUtils.GetSortableString(albumToInsert.AlbumTitle)) < 0; insertIndex++) ;
                                this.Albums.Insert(insertIndex, albumToInsert);
                                break;
                            case AlbumOrder.ByDateAdded:
                                for (insertIndex = 0; insertIndex < count && ((long)this.Albums[insertIndex].DateAdded).CompareTo(albumToInsert.DateAdded) > 0; insertIndex++) ;
                                this.Albums.Insert(insertIndex, albumToInsert);
                                break;
                            case AlbumOrder.ByDateCreated:
                                for (insertIndex = 0; insertIndex < count && ((long)this.Albums[insertIndex].DateFileCreated).CompareTo(albumToInsert.DateFileCreated) > 0; insertIndex++) ;
                                this.Albums.Insert(insertIndex, albumToInsert);
                                break;
                            case AlbumOrder.ByAlbumArtist:
                                for (insertIndex = 0; insertIndex < count && FormatUtils.GetSortableString(this.Albums[insertIndex].AlbumArtist).CompareTo(FormatUtils.GetSortableString(albumToInsert.AlbumArtist)) < 0; insertIndex++) ;
                                this.Albums.Insert(insertIndex, albumToInsert);
                                break;
                            case AlbumOrder.ByYearAscending:
                                for (insertIndex = 0; insertIndex < count && this.Albums[insertIndex].SortYear.CompareTo(albumToInsert.SortYear) < 0; insertIndex++) ;
                                this.Albums.Insert(insertIndex, albumToInsert);
                                break;
                            ...
                            case AlbumOrder.ByDateLastPlayed:
                                for (insertIndex = 0; insertIndex < count && ((long)this.Albums[insertIndex].DateLastPlayed).CompareTo(albumToInsert.DateLastPlayed) > 0; insertIndex++) ;
                                this.Albums.Insert(insertIndex, albumToInsert);
                                break;
                            default:
                                // Alphabetical
